    ...
    <section class="section-creative" style="padding: 60px 0;">
        ...
        <div class="card-creative p-4">
            ...
            <p class="text-muted mb-4">
                Просмотр и управление проектами. Одобряйте или отклоняйте проекты.
            </p>
            ...
            <div class="card-creative p-3 mb-3" style="background: var(--bg-card-hover); border: 1px solid var(--border-color); border-radius: 12px; box-shadow: 0 1px 4px rgba(0, 0, 0, 0.05);">
                <form method="GET" action="{{ route('admin.projects') }}" id="filterForm">
                ...
                <div class="col-12">
                            <div class="d-flex gap-3 align-items-center" style="padding-top: 8px; border-top: 1px solid var(--border-color);">
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <a href="{{ route('admin.projects') }}" class="btn btn-sm btn-outline-secondary px-3 py-1" style="border-radius: 8px; font-weight: 600; font-size: 0.875rem;">
                                    ...
                                    @if($project->city)
                                        <div class="d-flex align-items-center">
                                            <div style="font-size: 0.8rem;">
                                                ...
                                                @if($project->region)
                                                    <br><small class="text-muted" style="font-size: 0.75rem;">{{ $project->region->name }}</small>
                                                @endif
                                            </div>
                                        </div>
                                    @else
                                        ...
                                        <div class="d-flex flex-column align-items-center gap-1">
                                            ...
                                            @if($project->admin_comment)
                                                <div class="admin-comment admin-comment-rejected">
                                                    ...
                                                    <span style="font-size: 0.7rem;">{{ $project->admin_comment }}</span>
                                                </div>
                                            @endif
                                        </div>
                                ...
                                <td class="text-center" style="padding: 4px 8px;">
                                    <div class="d-flex gap-1 justify-content-center">
                                        ...
                                        <button
                                            ...
                                            data-bs-target="#revisionProjectModal"
                                            data-project-id="{{ $project->id }}"
                                            data-project-title="{{ $project->title }}"
                                            data-project-author="{{ $project->user->name }}"
                ...
                @empty
                            <tr>
                                ...
                                @endif
                                    ...
                                    @if(request()->hasAny(['search', 'status', 'region_id', 'city_id', 'date_from', 'date_to']))
                                        <a href="{{ route('admin.projects') }}" class="btn btn-sm btn-outline-primary mt-2">
            ...
            <div class="mt-3 text-muted text-center">
                <small>
                    ...
                    Показано {{ $projects->firstItem() ?? 0 }} - {{ $projects->lastItem() ?? 0 }} из {{ $projects->total() }} проектов
                </small>
            </div>
        </div>
        @else
...
<!-- Revision Project Modal -->
<div class="modal fade" id="revisionProjectModal" tabindex="-1" aria-labelledby="revisionProjectModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content card-creative" style="border: none;">
            <div class="modal-header" style="border-bottom: 1px solid var(--border-color);">
                ...
                <h5 class="modal-title fw-black" id="revisionProjectModalLabel">
                    Отправка на доработку
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="revisionProjectForm" method="POST">
                @csrf
                @method('PATCH')
                <div class="modal-body p-4">
                    <p class="lead mb-3">
                        Отправить проект <strong id="projectTitleToRevise"></strong> на доработку?
                    </p>
                    <div class="card-creative p-3 mb-3" style="background: rgba(107, 114, 128, 0.08); border: 1px solid rgba(107, 114, 128, 0.25);">
                        <p class="mb-1"><strong>Автор:</strong> <span id="projectAuthorToRevise"></span></p>
                    </div>
                    <div class="mb-3">
                        ...
                        <textarea
                            ...
                            name="comment"
                            rows="4"
                            placeholder="Укажите, что нужно исправить в проекте..."
                            required
...
<!-- Reject Project Confirmation Modal -->
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content card-creative" style="border: none;">
            <div class="modal-header" style="border-bottom: 1px solid var(--border-color);">
                ...
                <h5 class="modal-title fw-black" id="rejectProjectModalLabel">
                    Подтверждение отклонения проекта
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="rejectProjectForm" method="POST">
                @csrf
                @method('PATCH')
                <div class="modal-body p-4">
                    <p class="lead mb-3">
                        Вы уверены, что хотите отклонить проект <strong id="projectTitleToReject"></strong>?
                    </p>
                    <div class="card-creative p-3 mb-3" style="background: rgba(107, 114, 128, 0.08); border: 1px solid rgba(107, 114, 128, 0.25);">
                        <p class="mb-1"><strong>Автор:</strong> <span id="projectAuthorToReject"></span></p>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="alert alert-warning mb-0" style="border-radius: 12px; border: 1px solid var(--warning);">
                        ...
                        <strong>Внимание!</strong> Это действие нельзя отменить. Проект будет отклонен.
                    </div>
                </div>
                <div class="modal-footer" style="border-top: 1px solid var(--border-color);">
                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal" style="border-width: 1px; border-radius: 12px; font-weight: 600; padding: 12px 24px;">
...
document.addEventListener('DOMContentLoaded', function() {
    // Revision Modal
    const revisionProjectModal = document.getElementById('revisionProjectModal');
    const revisionProjectForm = document.getElementById('revisionProjectForm');
    const projectTitleToRevise = document.getElementById('projectTitleToRevise');
    const projectAuthorToRevise = document.getElementById('projectAuthorToRevise');

    if (revisionProjectModal) {
        revisionProjectModal.addEventListener('show.bs.modal', function (event) {
            const button = event.relatedTarget;
            const projectId = button.getAttribute('data-project-id');
            const projectTitle = button.getAttribute('data-project-title');
            const projectAuthor = button.getAttribute('data-project-author');

            projectTitleToRevise.textContent = projectTitle;
            projectAuthorToRevise.textContent = projectAuthor;
            revisionProjectForm.action = '{{ route("projects.revision", ":id") }}'.replace(':id', projectId);
        });
    }

    // Reject Modal
    const rejectProjectModal = document.getElementById('rejectProjectModal');
    const rejectProjectForm = document.getElementById('rejectProjectForm');
    const projectTitleToReject = document.getElementById('projectTitleToReject');
    const projectAuthorToReject = document.getElementById('projectAuthorToReject');

    if (rejectProjectModal) {
        rejectProjectModal.addEventListener('show.bs.modal', function (event) {
            const button = event.relatedTarget;
            const projectId = button.getAttribute('data-project-id');
            const projectTitle = button.getAttribute('data-project-title');
            const projectAuthor = button.getAttribute('data-project-author');

            projectTitleToReject.textContent = projectTitle;
            projectAuthorToReject.textContent = projectAuthor;
            rejectProjectForm.action = '{{ route("projects.reject", ":id") }}'.replace(':id', projectId);
        });
    }
