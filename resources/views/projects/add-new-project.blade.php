                        ...
                        @if(auth()->user()->can('add projects'))
                            <form method="POST" action="{{route('store-project')}}">
                                ...
                                <div class="mb-3">
                                    ...
                                    >
                                        <option value="" style="color: var(--text-secondary);">Выберите область</option>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select 
                                        ...
                                        disabled
                                    >
                                        <option value="" style="color: var(--text-secondary);">Сначала выберите область</option>
                                    </select>
                                </div>
                                    ...
                                    >
                                        ...
                                        @foreach($categoryRoots as $root)
                                            <optgroup label="{{ $root->name }}">
                                                <option value="{{ $root->id }}" {{ (string) old('category_id') === (string) $root->id ? 'selected' : '' }}>
                                                    Вся группа «{{ $root->name }}»
                                                </option>
                                                @foreach($root->children as $child)
                                                    <option value="{{ $child->id }}" {{ (string) old('category_id') === (string) $child->id ? 'selected' : '' }}>
                                                        {{ $child->name }}
                                                    </option>
                                                @endforeach
                                            </optgroup>
    ...
    regionSelect.addEventListener('change', function() {
        ...
        fetch(`/api/cities/${regionId}`)
            ...
            .then(cities => {
                ...
                placeholderOption.textContent = 'Выберите город';
                placeholderOption.style.color = 'var(--text-secondary)';
