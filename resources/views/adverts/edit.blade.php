    ...
    <section id="roles" class="section-creative" style="background: white;">
        <div class="container">
            ...
            <div class="text-center mb-5">
                <h1 class="display-6 fw-black mb-3">Редактирование объявление</h1>
            </div>
            <div class="row g-4">
                                    ...
                                    >
                                        ...
                                        @foreach($categoryRoots as $root)
                                            <optgroup label="{{ $root->name }}">
                                                <option value="{{ $root->id }}" {{ (string) old('category_id', $advert->category_id) === (string) $root->id ? 'selected' : '' }}>
                                                    Вся группа «{{ $root->name }}»
                                                </option>
                                                @foreach($root->children as $child)
                                                    <option value="{{ $child->id }}" {{ (string) old('category_id', $advert->category_id) === (string) $child->id ? 'selected' : '' }}>
                                                        {{ $child->name }}
                                                    </option>
                                                @endforeach
                                            </optgroup>
